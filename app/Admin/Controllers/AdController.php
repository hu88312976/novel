...
class AdController extends Controller
{
    ...
    protected function grid()
    {
        return Admin::grid(Ad::class, function (Grid $grid) {
            ...
            $grid->type('广告类型')->value(function ($type){
                if($type== 0){return '广告';}else{return '轮播';}
            });
            $grid->disableExport();
        });
    }
    ...
    protected function form()
    {
        return Admin::form(Ad::class, function (Form $form) {
            ...
            $form->image('image', "主图")->move("ad_images", $name.".jpg");


            $form->select('type','广告类型')->options(['0'=>'广告','1'=>'轮播']);

...
class HomeController extends Controller
{
    ...
    public function  Home(){
        $ad = new Ad();
        //广告
        $data['ad'] = $ad->getOne(['type'=>0],['link_address','image']);
        //轮播
        $data['ad_carousel']=  $ad->getLIst(['type'=>1],['link_address','image']);
        //如果页面没有静态化
        if (!file_exists('storage/upload/html/home.php'))
        {
            $htmlStrings = view('home',$data)->__toString();
            file_put_contents('storage/upload/html/home.php',$htmlStrings);
        }else{
            $htmlStrings = file_get_contents('storage/upload/html/home.php');
        }
        return $htmlStrings;
    }
}
